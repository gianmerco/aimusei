...
public class AccessibilityService {
	 ...
	 * 
	 * ETR: generazione testo ETR, gestione dello stato nel nostro db del testo
	 * semplificato
	 * 
	 * @param request
	 * @return
	...
	public TextGeneratedResponse generateSimplifiedTexts(TextGeneratedRequest request) {
		...
		if (operaByTagOPTLatest.isEmpty()) {
			Opera operaToInsert = Opera.builder().nome(request.getTitle()).descrizione(request.getOriginalText())
					.validator("gpt-4").version(0).hash(hash).tag(request.getTag()).lastUpdate(Instant.now())
		...
		if (!hash.equals(operaByTagOPTLatest.get().getHash())) {
			...
			Opera newOperaVersion = Opera.builder().nome(operaByTag.getNome()).descrizione(request.getOriginalText())
					.version(operaByTag.getVersion() + 1).hash(hash).tag(request.getTag()).lastUpdate(Instant.now())
					.latest(true).engineLLM("gpt-4").build();
	...
	public TextStatusResponse getTextStatus(String tag, String originalText) {
		...
		return TextStatusResponse.builder().hashMatch(hashMatch).textStatus(operaByHash.get().getStatoOpera()).build();
	}
	...
	public String regenerateSintesi(RegenerateSintesiRequest request) {
		...
		EtrRequest etrRequest = EtrRequest.builder()
				.content(StandardText.builder().value(opera.getDescrizione()).build()).build();
	...
	public byte[] generateImage(GenerateImageRequest request) {
		...
		String etr = sintesiService.createSintesi(request.getJson(), TextGeneratedRequest.Context.INFO_MUSEO)
				.getDescrizioneAI();
		...
		ByteArrayImage museumImage = ByteArrayImage.builder().value(downloadImageFromUrl(request.getImageUrl()))
				.build();

		PictogramsRequest pictogramsRequest = PictogramsRequest.builder().content(EtrText.builder().value(etr).build())
				.museumImage(museumImage).build();
