...
import it.prismaprogetti.aimusei.model.TextCheckRequest;
...
public class AccessibilityService {
	...
	public TextGeneratedResponse generateSimplifiedTexts(TextGeneratedRequest request) {
		...
		if(operaByTagOPT.isEmpty()) {
			Opera operaToInsert= Opera.builder()
					.nome(request.getTitle())
					.descrizione(request.getOriginalText())
					.validator("gpt-4")
					.version(0)
					.hash(hash)
					.tag(request.getTag())
					...
					.latest(true)
					.statoOpera(StatoOpera.INCOMPLETO)
					.engineLLM("gpt-4")
					.build();
		...
		if(!hash.equals(operaByTagOPT.get().getHash())) {
			...
			Opera newOperaVersion = Opera.builder()
					.nome(operaByTag.getNome())
					.descrizione(request.getOriginalText())
					.version(operaByTag.getVersion()+1)
					.hash(hash)
					.tag(request.getTag())
					.lastUpdate(LocalDate.now())
					.latest(true)
		...
		// hash uguale, ritorno quello esistente, TODO prima effettuo un retry di generazione sintesi nel caso non fossero tutte presenti
	...
	public TextOriginalResponse getOriginalText(String tag, 
		    String originalText) {
	...
	private List<Sintesi> createSintesi(String prompt,List<TipoDisabilita> toExclude) throws SintesiWrapperException {
		...
		List<TipoDisabilita> tipiDisabilita = new ArrayList<>(
				Arrays.asList(TipoDisabilita.values()));
	    ...
	    for (TipoDisabilita tipo : tipiDisabilita) {
	        try {
	            ...
	            throw new SintesiWrapperException(
	                "Errore durante la generazione della sintesi per " + tipo + 
	                ". Sintesi generate: " + sintesiList.size(),
	...
	private void updateSintesi(String hash, TipoDisabilita tipoDisabilita, String descrizioneReviewed) {
	    operaRepository.findByHash(hash)
	        .ifPresent(opera -> {
